# Nivare - App Store Listing

## App Name
Nivare

## Subtitle (30 chars)
Your Voice, Finally Heard

## Keywords (100 chars)
mental health,AI companion,ADHD,mood,therapy,wellness,journal,impulse tracker,voice,focus,dopamine

## Short Description (80 chars - Google Play)
AI companion for mental health, ADHD focus, mood tracking & impulse control.

## Full Description

Nivare is an AI companion that truly listens. More than a chatbot — Nivare understands your emotions, remembers your story, and adapts every conversation to how you're feeling right now. Built with ADHD and neurodivergent minds in mind.

MOOD-AWARE CONVERSATIONS
Nivare detects your emotional state through conversation patterns and responds with empathy. Having a rough day? It offers grounding exercises and calming support. Feeling good? It matches your energy.

ADHD FOCUS TOOLS (New!)
Designed specifically for neurodivergent brains:
- Energy-based suggestions — tell Nivare if you're Low, Medium, or High energy and get adapted recommendations
- Adaptive chat responses — shorter messages when you're drained, deeper when you're focused
- Daily action icons — small tasks (breathe, journal, coping skill, energy check, focus, impulse track) that disappear one by one as you complete them
- Dopamine reward streaks — variable-ratio rewards for completing daily actions, just like the psychology that keeps games engaging, but for healthy habits
- Body double nudges — gentle daily check-in reminders that act as external accountability

FINANCIAL IMPULSE TRACKER (Pro)
ADHD and impulsive spending go hand in hand. Nivare helps you:
- Log spending urges in the moment
- Mark whether you resisted or gave in
- Track your resist rate and money saved over time
- Earn reward points for every impulse you resist

FOCUS TOGETHER (New!)
A silent "body-double" focus session — Nivare sits with you while you work. A calm timer, gentle presence cues, and zero pressure. Stopping early still counts. Built to make starting tasks easier.

PLAY ANY SONG (New!)
Just ask — "play Sidhu Moose Wala" or "play sad Punjabi songs" — in any language. Nivare finds it and opens it in Spotify. Works with Hindi, Punjabi, English and more.

SPOTIFY MOOD MUSIC (Pro)
Connect your Spotify account and let Nivare suggest playlists that match your current mood. Anxious? Calming lo-fi. Energetic? Upbeat vibes.

VOICE INTERACTION
Talk to Nivare hands-free with a wake word and natural voice responses. Name your assistant anything you like — it answers to the name you give it.

MEMORY & PERSONALISATION
Unlike generic chatbots, Nivare remembers your conversations and builds understanding over time. It knows what matters to you and uses that context to give responses that actually help.

WELLBEING & COPING TOOLS
Self-guided tools for everyday reflection — inspired by widely-used wellbeing techniques:
- Journaling and thought records
- Grounding and breathing exercises
- Coping-skill prompts
- Crisis resource links

PRIVATE & SECURE
Your sensitive data — moods, focus, impulse logs — stays on your device. No ads, no data brokers. Delete everything anytime.

IMPORTANT
Nivare is a wellbeing and self-reflection companion, not a medical or therapy service. It does not diagnose, treat, or cure ADHD or any condition, and is not a substitute for professional care. In a crisis, contact local emergency services or a crisis line.

FREE PLAN
- 250 messages per month
- 30 wake word activations per month
- 7-day chat history
- Daily ADHD action icons
- Energy-based suggestions
- Dopamine reward streaks

PRO PLAN ($14.99/month or $168/year)
- 3,500 messages per month
- Unlimited wake words
- Spotify mood detection
- Personalised AI responses
- Unlimited chat history
- ADHD Focus Tools (full suite)
- Financial Impulse Tracker
- Adaptive chat response length

Download Nivare and finally feel heard.

## Privacy Policy URL
https://upkarnz.github.io/nivare-website/privacy.html

## Terms of Service URL
https://upkarnz.github.io/nivare-website/terms.html

## Support URL
https://upkarnz.github.io/nivare-website/#support

## Support Email
support@nivare.app

## Age Rating Answers

Category: Health & Fitness / Lifestyle / ADHD Support

### Apple Age Rating Questionnaire:
- Cartoon or Fantasy Violence: None
- Realistic Violence: None
- Prolonged Graphic or Sadistic Realistic Violence: None
- Profanity or Crude Humor: None
- Mature/Suggestive Themes: None
- Horror/Fear Themes: None
- Medical/Treatment Information: Infrequent/Mild (mental health tools)
- Simulated Gambling: None
- Real Gambling: None
- Contests: None
- Unrestricted Web Access: None
- Alcohol, Tobacco, or Drug Use: None
- Sexual Content or Nudity: None

Result: Rated 4+ (or 12+ due to medical/treatment info)

### Google Play Content Rating (IARC):
- Violence: No
- Sexual content: No
- Language: No
- Controlled substance: No
- User interaction: Yes (AI chat)
- Shares personal info: No
- Shares location: No (optional, user-controlled)
- Digital purchases: Yes (subscriptions)

Result: Everyone / E (PEGI 3 / USK 0)

### Privacy Declarations (both stores):
- Data collected: Email, usage data, conversation content
- Data linked to user: Email, conversation history
- Data NOT collected/transmitted: Energy levels, impulse tracker entries, streak data (stored locally only)
- Data used for tracking: None
- Third-party analytics: Firebase Analytics (anonymised)
